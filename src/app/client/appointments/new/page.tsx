...
import { Calendar as CalendarIcon, Clock, Scissors, LogIn, ArrowLeft } from 'lucide-react';
...
export default function NewAppointmentPage() {
  ...
  useEffect(() => {
    ...
    // Captura parâmetros da URL se o usuário veio redirecionado do login ou de outro link
  ...
  // Horários operacionais baseados nas suas regras de terça a domingo
  const timeSlotsForSelectedDay = useMemo(() => {
    if (!selectedDate) return [];
    const dayOfWeek = getDay(selectedDate);
    
    if (dayOfWeek === 1) return []; // Segunda fechado
  ...
  // Processa travas de agendamento de 10 minutos

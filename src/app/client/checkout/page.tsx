  ...
  useEffect(() => {
    if (serviceId) {
      ...
      if (found) {
        ...
        setError("Serviço selecionado não foi encontrado.");
      }
    } else {
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      if (checkoutUrl) {
        ...
        throw new Error("Não foi possível carregar o gateway de pagamento.");
      }
    } catch (err: any) {
            ...
            <div className="flex-1 w-full text-center sm:text-left space-y-1">
              ...
              <div className="flex items-center justify-center sm:justify-start gap-3 pt-2 text-xs text-muted-foreground">
                <div className="flex items-center gap-1">
                  ...
                  <span>{dateStr ? dateStr.split('-').reverse().join('/') : ''}</span>
                </div>
                <div className="flex items-center gap-1">
                  <Clock className="w-3.5 h-3.5 text-primary" />
                  <span>{timeStr || service.duration || "30 min"}</span>
                </div>
                <div className="flex items-center gap-1">

...
function SucessoContent() {
  ...
  const irParaReservas = () => {
    const destino = orderNsu ? `/client/appointments?status=success&order_nsu=${orderNsu}` : '/client/appointments';
    router.push(destino);
  };
  ...
  return (
    <Card className="w-full max-w-md border-primary/20 shadow-xl">
      <CardHeader className="text-center pb-2">
        ...
        <CardTitle className="text-2xl font-bold font-headline text-foreground">Pagamento Confirmado!</CardTitle>
        <p className="text-sm text-muted-foreground mt-1">Seu agendamento foi realizado e o pagamento processado com sucesso.</p>
      </CardHeader>
      ...
      <CardFooter className="flex flex-col gap-2 pt-2">
        <Button className="w-full gap-2 font-semibold" onClick={irParaReservas}>
          <FaCalendarAlt className="w-4 h-4" /> Ver Minhas Reservas
        </Button>
        <Button variant="ghost" className="w-full text-xs text-muted-foreground hover:text-foreground" onClick={() => router.push('/')}>
          Voltar ao Início <FaArrowRight className="w-3 h-3 ml-1" />
        </Button>
      </CardFooter>
    </Card>
  );
...
export default function CheckoutSucessoPage() {
  return (
    <main className="min-h-[calc(100vh-4rem)] w-full flex items-center justify-center p-4 bg-background">
      <Suspense fallback={<div className="text-center text-muted-foreground animate-pulse">Carregando confirmação...</div>}>
        <SucessoContent />
      </Suspense>
    </main>
  );
